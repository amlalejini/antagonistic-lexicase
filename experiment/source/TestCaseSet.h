...
public:
    ...
    TestCaseSet() {
        fun_load_test_case = [](const std::string &) { return {input_t(), output_t()}; }
    }

    test_case_t & operator[](size_t id) { return test_cases[id]; }
    const test_case_t & operator[](size_t id) { return test_cases[id]; }
    ...
    /// Get test case set
    emp::vector<test_case_t> & GetTestCaseSet() {
        return test_cases;
    }
